...
def process_in_chunks(rasterio_dataset, index_folder, chunk_size=512):
    ...
    with rasterio.open(os.path.join(index_folder, 'vari_image.tif'), 'w',
                       **profile) as vari_dest, \
            rasterio.open(os.path.join(index_folder, 'gli_image.tif'), 'w',
                          **profile) as gli_dest:

        for i in range(0, height, chunk_size):
            for j in range(0, width, chunk_size):
                # Define the window to read the chunk
                window = rasterio.windows.Window(j, i,
                                                 min(chunk_size, width - j),
                                                 min(chunk_size, height - i))

                # Read chunks of the red and nir bands
                red_band = rasterio_dataset.read(band_mapping['red'],
                                                 window=window).astype(
                    'float32')
                green_band = rasterio_dataset.read(band_mapping['green'],
                                                   window=window).astype(
                    'float32')
                blue_band = rasterio_dataset.read(band_mapping['blue'],
                                                  window=window).astype(
                    'float32')

                # Calculate NDVI for the chunk
                vari = np.where((green_band + red_band - blue_band) == 0,
                                np.nan,
                                (green_band - red_band) / (
                                        green_band + red_band - blue_band))
                vari[np.isnan(vari)] = np.nan
                ...
                gli = np.where((2 * green_band + red_band + blue_band) == 0,
                               np.nan,
                               (2 * green_band - red_band - blue_band) / (
                                       2 * green_band + red_band + blue_band))
                gli[np.isnan(gli)] = np.nan
