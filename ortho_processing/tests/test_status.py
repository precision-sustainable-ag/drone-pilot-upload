...
def call_recompute(odm, oi, prev, failed_now=False):
    if HAS_FAILED_NOW:
        return recompute_overall(odm, oi, prev, failed_now=failed_now)
    return recompute_overall(odm, oi, prev)

def test_reconcile_state_tail_precedence():
    s, why = reconcile_state(prev="processing", ok_by_artifacts=False, tail_state="succeeded", tail_reason="footer")

    assert s == "succeeded"
    s2, _ = reconcile_state(prev="succeeded", ok_by_artifacts=False, tail_state="failed", tail_reason="exit")
    assert s2 == "failed"
...
def test_reconcile_state_sticky_terminal_without_new_evidence():
    s, why = reconcile_state(prev="failed", ok_by_artifacts=False, tail_state=None, tail_reason=None)
    assert s == "failed"
    s2, _ = reconcile_state(prev="succeeded", ok_by_artifacts=False, tail_state=None, tail_reason=None)
    assert s2 == "succeeded"
...
def test_recompute_overall_failure_rule():
    if HAS_FAILED_NOW:
        ...
        assert call_recompute(odm="succeeded", oi="failed", prev="processing", failed_now=True) == "failed"
        assert call_recompute(odm="succeeded", oi="failed", prev="processing", failed_now=False) == "processing"
    else:
...
def test_recompute_no_promotion_on_stale_oi_failure():
    ...
    assert call_recompute(odm="succeeded", oi="failed", prev="ortho generated", failed_now=False) == "ortho generated"

def test_recompute_no_demote_from_ortho_generated_on_stale_failure():
    # When we were already "ortho generated", and OI is marked failed but there's no fresh tail failure,
    ...
    def call(odm, oi, prev, failed_now=False):
        return recompute_overall(odm, oi, prev, failed_now=failed_now) if HAS_FAILED_NOW else recompute_overall(odm, oi, prev)

    assert call(odm="succeeded", oi="failed", prev="ortho generated", failed_now=False) == "ortho generated"
