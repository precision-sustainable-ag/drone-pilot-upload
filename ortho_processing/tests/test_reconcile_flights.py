...
def neutral_tails(monkeypatch):
    ...
    monkeypatch.setattr(rf, "lsf_terminal_status", lambda *a, **k: (None, "no terminal marker"))


# --- tests ------------------------------------------------------------------

def test_file_count_mismatch_blocks_and_sets_status(make_flight, patch_config, patch_records_connect_db, fake_db, monkeypatch):
    f = make_flight("C1", n_images=5)
    meta = _meta("C1", num_files=6, status="processing")

    # Ensure no other side effects run
    monkeypatch.setattr(rf, "odm_done", lambda d: (False, "no"))
    monkeypatch.setattr(rf, "ortho_intel_done", lambda d: (False, "no"))
    monkeypatch.setattr(rf, "lsf_terminal_status", lambda *a, **k: (None, ""))
...
def test_odm_success_promotes_to_ortho_generated(make_flight, patch_config, patch_records_connect_db, fake_db, neutral_tails):
    ...
    assert patch_records_connect_db.docs["S1"]["orthophoto_path"].endswith("central/flights/S1/odm_orthophoto/odm_orthophoto.tif")

    # We should have a stage update to succeeded (flattened path)
    updates = [u for (fid, u) in patch_records_connect_db.updates if fid == "S1"]
    flat = {}
...
def test_ortho_intel_success_yields_processed(make_flight, patch_config, patch_records_connect_db, neutral_tails):
...
def test_fresh_tail_failure_drops_to_failed(make_flight, patch_config, patch_records_connect_db, monkeypatch):
...
def test_stale_failure_does_not_downgrade_overall(make_flight, patch_config, patch_records_connect_db, monkeypatch):
...
def test_processed_is_terminal(make_flight, patch_config, patch_records_connect_db, monkeypatch):
    f = make_flight("T1", with_code=True)
    meta = _meta("T1", status="processed", stages={"odm": {"state": "succeeded"}, "ortho_intel": {"state": "succeeded"}})

    # Even if tails now say failed, processed overall should not downgrade
    monkeypatch.setattr(rf, "lsf_terminal_status", lambda *a, **k: ("failed", "Exited with exit code 137."))
    monkeypatch.setattr(rf, "odm_done", lambda d: (True, "log success"))
    monkeypatch.setattr(rf, "ortho_intel_done", lambda d: (True, "veg present"))
...
def test_dry_run_writes_nothing(make_flight, patch_config, patch_records_connect_db, monkeypatch):
