...
def generateLsfScript(flight_dir, process_name, ortho_file=None):
    if process_name == "odm":
        odm_sif_file = os.path.join(config["code_dir"], "sif_files", "odm_gpu-fixed.sif")
        ...
        ortho_intel_sif_file = os.path.join(
            config["code_dir"], "sif_files", "drone_ortho_intel.sif"
        )
        with open(lsfScript, "w") as file:
            file.write(f"""#!/bin/bash
        #BSUB -n 32
...
def processFlight(flight_id):
    ...
    if flight_metadata["num_files"] == utils.countFiles(os.path.join(flight_dir, "images")):
        odm_script = generateLsfScript(flight_dir, "odm")
        utils.updateRecord(flight_dir, flight_id, "processing")
        job_id = utils.lsfSubmitJob(odm_script, flight_dir)
        logging.info(
            {"service": "processFlight", "message": f"{flight_id} - odm job submitted - {job_id}"}
        )
        ...
        if status != "EXIT":
            ...
            if not os.path.exists(os.path.join(code_dir, "log.json")):
                logging.error(
                    {"service": "processFlight", "message": f"{flight_id} - odm log file not there"}
                )
                utils.updateRecord(flight_dir, flight_id, "failed")
                return None
            with open(os.path.join(code_dir, "log.json")) as log_file:
                ...
                ortho_intel_script = generateLsfScript(flight_dir, "ortho_intel", ortho_file)
                job_id = utils.lsfSubmitJob(ortho_intel_script, flight_dir)
                logging.info(
                    {
                        "service": "processFlight",
                        "message": f"{flight_id} - ortho intel job submitted - {job_id}",
                    }
                )
                status = utils.lsfMonitorJob(job_id, flight_id)
                if status != "EXIT":
        ...
        logging.info(
            {"service": "processFlight", "message": f"{flight_id} - file count not matching"}
        )
        return None
...
def main():
    ...
    for row in results:
        ...
        elif row["status"] not in ["processed", "processing", "ortho generated"]:
            records_to_process.append(row["flight_id"])
